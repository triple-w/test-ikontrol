...
<div class="p-4 sm:p-6" 
     x-data="{
        ...
        close(){ this.modal=false; this.facturaId=null; this.toEmail=''; this.ccEmail=''; },
        actionUrl(){ return `{{ url('/facturacion/facturas') }}/${this.facturaId}/email`; }
     }">

  <div class="flex items-center justify-between mb-4">
    <h1 class="text-xl font-semibold">Facturas (RFC: {{ session('rfc_seleccionado') ?? '—' }})</h1>
      ...
      <table class="min-w-full text-sm">
        <thead>
          <tr class="text-left text-gray-500 border-b">
            <th class="py-2 pr-4">Serie–Folio</th>
        ...
        <tbody class="divide-y">
          @forelse($items as $f)
            ...
            <tr class="hover:bg-gray-50">
              ...
              <td class="py-2 pr-4">
                {{ $f->total_xml !== null ? number_format($f->total_xml, 2) : '—' }}
              </td>
              <td class="py-2 pr-4 capitalize">{{ $f->tipo_comprobante ?? '—' }}</td>
              <td class="py-2 pr-4">
                {{ $f->razon_social }} <span class="text-gray-500">({{ $f->rfc }})</span>
              </td>
              <td class="py-2 pr-4">{{ optional($f->fecha_factura)->format('Y-m-d H:i') ?? '—' }}</td>
              <td class="py-2 pr-4">{{ $f->estatus }}</td>
              <td class="py-2 pr-4">
                <div class="flex gap-3">
                  <a href="{{ route('facturas.show', $f) }}" class="text-violet-600 hover:underline text-sm">Ver</a>
                  <a href="{{ route('facturas.pdf', $f) }}" class="text-violet-600 hover:underline text-sm">PDF</a>
                  <a href="{{ route('facturas.xml', $f) }}" class="text-violet-600 hover:underline text-sm">XML</a>
                  <button type="button"
                          class="text-violet-600 hover:underline text-sm"
                          @click="openEmail({{ $f->id }}, '{{ e($emailsPorRfc[$f->rfc] ?? '') }}')">
                    Enviar
                  </button>
                </div>
              </td>
          ...
          @empty
            <tr><td colspan="6" class="py-6 text-center text-gray-500">Sin facturas.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
  ...
  <div x-show="modal" x-cloak
    ...
    <div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-xl shadow p-6">
      <h2 class="text-lg font-semibold mb-4">Enviar factura por correo</h2>
