...
<div class="relative inline-flex" x-data="{ open: false }">
    ...
    >
        @php
            ...
            $rfcActivo = session('rfc_seleccionado', $rfcs->first()?->rfc ?? 'Sin RFC');
            $razonSocial = $rfcs->firstWhere('rfc', $rfcActivo)?->razon_social ?? 'Sin razón social';
        @endphp

        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-gray-200 dark:border-gray-700/60">
            <div class="font-medium text-gray-800 dark:text-gray-100">{{ Auth::user()->name }}</div>
            <div class="text-xs text-gray-500 dark:text-gray-400 italic">{{ $rfcActivo }}</div>
            <div class="text-[10px] text-gray-400 italic">{{ $razonSocial }}</div>
        </div>

        <ul>
            @if($rfcs->count() > 1)
                <li class="border-t border-gray-200 dark:border-gray-700/60 mt-1 pt-1 px-3">
                    <div class="text-[11px] text-gray-400 mb-1">Cambiar RFC</div>
                    @foreach($rfcs as $rfc)
                        <form method="POST" action="{{ route('rfc.cambiar') }}" class="mb-1">
                            @csrf
                            <input type="hidden" name="rfc_id" value="{{ $rfc->id }}">
                            <button type="submit" class="text-xs text-left text-gray-700 hover:text-violet-600 dark:text-gray-300 dark:hover:text-violet-400">
                                {{ $rfc->rfc }} <br>
                                <span class="text-[10px] italic text-gray-400">{{ $rfc->razon_social }}</span>
                            </button>
                        </form>
                    @endforeach
                </li>
