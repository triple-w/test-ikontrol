...
use App\Services\TimbradorXpress;
...
class FacturaUiController extends Controller
{
    ...
    public function preview(Request $request)
    {
        ...
        Validator::make($payload, [
            ...
            'uso_cfdi' => 'required|string|max:4', // Validación agregada
    ...
    public function guardar(Request $r)
    {
        ...
        $b->uso_cfdi = $payload['uso_cfdi'] ?? null; // Asegurar que se guarde si tienes columna, o en payload
     ...
     */
    public function timbrar(Request $r, TimbradorXpress $txp)
    {
        // 1) Decodificar payload (tolerante a &quot;)
        $payload = $this->decodePayloadFromRequest($r);
        if (empty($payload) || !isset($payload['cliente_id'])) {
            return back()->with('error', 'Payload inválido para timbrar.');
        }

        // 2) Validación mínima (uso_cfdi debe venir en "Datos del comprobante")
        if (empty($payload['uso_cfdi'])) {
            return back()->with('error', 'Falta UsoCFDI en los datos del comprobante.');
        }

        $rfcUsuarioId = (int) session('rfc_usuario_id');
        if (!$rfcUsuarioId) {
            // si no está en sesión, intentar localizar por rfc seleccionado
            $rfc = session('rfc_seleccionado');
            if ($rfc) {
                $rfcUsuarioId = (int) DB::table('rfc_usuarios')->where('rfc', $rfc)->value('id');
            }
        }
        if (!$rfcUsuarioId) {
            return back()->with('error', 'No hay RFC activo seleccionado.');
        }

        try {
            // 3) Timbrar (REST dev)
            $res = $txp->timbrarJSON2($payload, $rfcUsuarioId, (int) $payload['cliente_id'], null);
            $uuid = $res['uuid'];
            $xml = $res['xml'];
            $pdfB64 = $res['pdf_b64'] ?? null;

            // 4) Guardar en tabla facturas (mapeando tus columnas)
            $cliente = DB::table('clientes')->where('id', $payload['cliente_id'])->first();
            $emisor = DB::table('rfc_usuarios')->where('id', $rfcUsuarioId)->first();

            $fechaFactura = $payload['fecha'] ?? now();

            DB::table('facturas')->insert([
                'users_id' => auth()->id(),
                'rfc_usuario_id' => $rfcUsuarioId,
                'rfc' => $cliente->rfc ?? '',
                'razon_social' => $cliente->razon_social ?? '',
                'calle' => $cliente->calle ?? null,
                'no_ext' => $cliente->no_ext ?? null,
                'no_int' => $cliente->no_int ?? null,
                ...
                'nombre_contacto' => $cliente->nombre_contacto ?? null,
                'estatus' => 'timbrada',
                'id_cancelar' => null,
                'fecha' => now(),
                'uso_cfdi' => $payload['uso_cfdi'] ?? null, // Guardar en tabla facturas
                'fecha_factura' => $fechaFactura,
                'xml' => $xml,
                'pdf' => $pdfB64,          // lo guardas como base64 en DB según tu diseño actual
                'solicitud_timbre' => null,
                'acuse' => null,
                'descuento' => (float) ($payload['descuento'] ?? 0), // si no lo traes, lo calcularías igual que en preview
                'uuid' => $uuid,
                'nombre_comprobante' => ($payload['serie'] ?? '') . '-' . ($payload['folio'] ?? ''),
                'tipo_comprobante' => $payload['tipo_comprobante'] ?? 'I',
                'comentarios_pdf' => $payload['comentarios_pdf'] ?? null,
                'created_at' => now(),
                'updated_at' => now(),
            ]);

            return redirect()->route('facturas.index')
                ->with('ok', 'Timbrado correcto. UUID: ' . $uuid);

        } catch (\Throwable $e) {
            Log::error('Error de timbrado', ['ex' => $e->getMessage()]);
